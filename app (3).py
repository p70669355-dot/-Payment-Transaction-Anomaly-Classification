...
# ============================================================
    ...
    st.write(
        "Analyze an individual payment transaction using the trained machine-learning model."
    )
    ...
    if st.button(
        ...
        try:
            ...
            if prediction == 1:
                ...
                st.error(
                    "🚨 SUSPICIOUS / ANOMALOUS TRANSACTION"
                )

                st.write(
                    "The trained machine-learning model classified "
                    "this transaction as anomalous."
                )

            else:

                st.success(
                    "✅ NOT SUSPICIOUS / ROUTINE TRANSACTION"
                )

                st.write(
                    "The trained machine-learning model classified "
                    "this transaction as routine."
                )
            ...
            if probability is not None:
                ...
                st.caption(
                    "This score represents the model's estimated "
                    "anomalous-class probability for this transaction; "
            ...
            st.error(
                ...
                "Please place final_model.pkl in the same folder as app.py."
            )

        except Exception as e:

            st.error(
                f"Unable to analyze the transaction: {e}"
            )
